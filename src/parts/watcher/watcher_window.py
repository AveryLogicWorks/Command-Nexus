...
class WatcherWindow(QMainWindow):
    ...
    def __init__(self, registry=None, audit=None, engine: WatcherEngine | None = None):
        ...
        is_dev = self._mode_text == "dev"
        if is_dev:
            self._state.active = False
            self._status_label.setText("DEV MODE")
            self._status_label.setStyleSheet("color: #58a6ff; font-weight: bold; font-size: 16px;")
            self._log_alert(AlertSeverity.INFO, "mode", "Development mode — monitoring inactive.", "system", "Log-only mode")
        else:
            self._init_baseline()
    ...
    def _init_baseline(self):
        """Establish baseline hashes for protected source files only."""
        project_root = Path(__file__).resolve().parent.parent.parent
        protected = []
        for pattern in TripwireManager.PROTECTED_PATTERNS:
    ...
    def _toggle_monitoring(self):
        btn = self.sender()
        if self._mode_text == "dev":
            QMessageBox.information(self, "DEV Mode", "Monitoring is inactive in development mode. No scanning or alerting is active.")
            return
    ...
    def _force_scan(self):
        if self._mode_text == "dev":
            QMessageBox.information(self, "DEV Mode", "Scanning is inactive in development mode.")
            return
        self._scan_cycle()
