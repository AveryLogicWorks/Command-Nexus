...
class BookWindow(QMainWindow):
    ...
    book_saved = Signal(str, str)  # ai_uuid, ai_name
    defaults_edited = Signal(str, bool)  # ai_uuid, edited
    command_to_ai = Signal(str)  # command text — routed to AI, memory NEVER included
    ...
    def _open_book_ai_dialog(self):
        ...
        use_case = ""
        if self._registry:
            meta = self._registry.get(self._current_ai_uuid)
            if meta:
                abilities = meta.get("abilities", [])
                use_case = meta.get("use_case", "")
        dialog = KnowledgeAIDialog(
            ai_name, self._current_ai_uuid, existing_context,
            parent=self, abilities=abilities, use_case=use_case,
        )
