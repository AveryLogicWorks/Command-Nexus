...
from ...core.resource_gate import get_resource_gate, GateDecision
...
def _scaffold_unit(unit: AIUnit, purpose: str = "", base_dir: Path | None = None) -> AIUnit:
    ...
    try:
        ...
        raise RuntimeError(f"Cannot create archive base directory '{base}': {e}") from e
    folder = base / f"{unit.name.replace(' ', '_')}_{unit.uuid}"
    try:
        folder.mkdir(parents=True, exist_ok=True)
    except OSError as e:
        raise RuntimeError(f"Cannot create unit folder '{folder}': {e}") from e
    ...
    try:
        ...
        pass  # Non-fatal: migration/backup failure should not block scaffolding
    ...
    if not book_path.exists() or not unit.book_defaults_edited:
        try:
            ...
            raise RuntimeError(f"Cannot write ability book '{book_path}': {e}") from e
    ...
    try:
        ...
        pass  # Non-fatal: profile persistence failure should not block scaffolding
    return unit
...
USE_CASE_OPTIONS: dict = {
    UseCaseClass.INDIVIDUAL: [
        ...
        "Tool User",  # Available in all use cases
        # Premium upgrades
    ...
    UseCaseClass.TASK_READY: [
        ...
        "Curriculum Designer", "Exam Prep Coach", "Academic Citation Manager",
    ],
    UseCaseClass.BUSINESS: [
        ...
        "Report Generator", "Invoice Processor", "PR Assistant",
        "Internal Comms Writer", "Market Analyst", "Brand Strategist",
        "Presentation Coach",
    ],
    UseCaseClass.ENTERPRISE: [
        ...
        "Privacy Compliance Checker", "Data Governance Advisor",
        "DevOps Assistant", "Database Manager",
    ],
    UseCaseClass.FINANCIAL_GAINER: [
        ...
        "Personal Finance Manager", "Market Analyst", "Trend Forecaster",
    ],
    ...
    UseCaseClass.GAMING: [
        "Game Companion", "Chat Companion", "Research Assistant",
        "Customer Support AI",
        "Tool User",
        # Premium upgrades
        "Smart Search", "Visual Canvas", "Voice Interface",
        "Activity Watcher", "Memory Recorder",
    ],
    UseCaseClass.DEVELOPER_CREATOR: [
        "Chat Companion",
        "Coding Assistant", "Code Reviewer", "Creative Writer",
        "Research Assistant", "Document Processor",
        "Customer Support AI",
        "Tool User",
        # Premium upgrades
        "API Integrator", "Data Analyst Pro", "Security Auditor",
        "Smart Search", "Knowledge Base Builder", "Email Automation",
        "Social Media Manager", "Presentation Builder", "Content Strategist",
        "DevOps Assistant", "Database Manager", "Test Generator",
        "Documentation Generator", "Script Writer",
    ],
    UseCaseClass.HEALTH_WELLNESS: [
        "Wellness Coach", "Chat Companion", "Personal Organizer",
        "Calendar Manager",
        "Customer Support AI",
        "Tool User",
        # Premium upgrades
        "Habit Tracker", "Progress Journal", "Activity Watcher",
        "Memory Recorder", "Smart Search", "Notebook",
        "Recipe Planner", "Personal Finance Manager",
    ],
    UseCaseClass.CREATIVE_STUDIO: [
        "Chat Companion",
        "Content Strategist", "Creative Writer", "Marketing Generator",
        "Social Media Manager", "Presentation Builder",
        "Customer Support AI",
        "Tool User",
        # Premium upgrades
        "Visual Canvas", "Voice Interface", "Document Generator",
        "Smart Search", "Research Assistant", "Email Automation",
        "Knowledge Base Builder", "Game Companion",
        "Podcast Planner", "Brand Strategist", "Copy Editor",
    ],
    UseCaseClass.ALL_ROUNDER: [
        ...
        "Wellness Coach", "Content Strategist",
        # Phase 7 — new capabilities
        "Task Scheduler", "Form Builder", "Report Generator",
        "Invoice Processor", "Spreadsheet Analyst", "Data Visualizer",
        "Statistical Modeler", "Trend Forecaster", "DevOps Assistant",
        "Database Manager", "Test Generator", "Documentation Generator",
        "Script Writer", "Copy Editor", "Podcast Planner",
        "Brand Strategist", "Presentation Coach", "PR Assistant",
        "Internal Comms Writer", "Academic Citation Manager", "Patent Researcher",
        "Market Analyst", "Recipe Planner", "Travel Planner",
...
USE_CASE_DESCRIPTIONS: dict = {
    ...
    UseCaseClass.GAMING: "Game-focused AI that learns board games and card games, teaches rules, suggests strategies, analyzes positions, and plays practice games. For entertainment and skill-building — no gambling.",
    UseCaseClass.DEVELOPER_CREATOR: "AI for developers and content creators. Combines coding assistance, code review, creative writing, research, and content strategy into one powerful workflow.",
    UseCaseClass.HEALTH_WELLNESS: "Wellness-focused AI with fitness planning, nutrition guidance, mental wellness support, habit building, and progress tracking. Advisory only — not a substitute for professional medical advice.",
    UseCaseClass.CREATIVE_STUDIO: "Content creation powerhouse with content calendar planning, audience analysis, platform optimization, content repurposing, and brand voice development. For creators who want to publish consistently and strategically.",
...
USE_CASE_RECOMMENDED: dict = {
    ...
    UseCaseClass.GAMING: ["Game Companion", "Chat Companion", "Research Assistant", "Smart Search"],
    UseCaseClass.DEVELOPER_CREATOR: ["Coding Assistant", "Code Reviewer", "Content Strategist", "API Integrator"],
    UseCaseClass.HEALTH_WELLNESS: ["Wellness Coach", "Habit Tracker", "Progress Journal", "Calendar Manager"],
    UseCaseClass.CREATIVE_STUDIO: ["Content Strategist", "Creative Writer", "Social Media Manager", "Visual Canvas"],
...
CAPABILITY_DESCRIPTIONS: dict[str, str] = {
    ...
    "Advanced Memory System": "Enhanced memory with cross-session recall, context bridging, and intelligent forgetting. Remembers what matters and discards what doesn't.",
    "Custom Model Connector": "Connect your own local or cloud LLM model. Supports GGUF, Ollama, LM Studio, and custom API endpoints for full model control.",
    # Wellness Coach capabilities
    "Wellness Coach": "Plans fitness routines, suggests meals, guides mental wellness exercises, builds habits, and tracks progress. Advisory only — not medical advice. Always consult a healthcare professional.",
    # Content Strategist capabilities
    "Content Strategist": "Plans content calendars, analyzes audiences, optimizes for platforms, repurposes content across channels, and defines brand voice. You approve everything before publishing.",
    # Phase 7 — new capability descriptions
    "Task Scheduler": "Suggests time blocks, scheduling plans, and reminder strategies. Does not auto-schedule or modify calendars.",
    "Form Builder": "Designs forms, surveys, and questionnaires with appropriate question types and structure.",
    "Report Generator": "Creates structured reports with executive summaries, findings, and recommendations from your input.",
    "Invoice Processor": "Generates formatted invoices with calculations and payment terms. Does not send invoices or process payments.",
    "Spreadsheet Analyst": "Provides spreadsheet formulas, pivot table guidance, and data analysis help.",
    "Data Visualizer": "Recommends chart types and visualization strategies based on your data and goals.",
    "Statistical Modeler": "Guides regression, hypothesis testing, and statistical analysis with method selection and interpretation.",
    "Trend Forecaster": "Analyzes historical data patterns and projects future trends with confidence intervals.",
    "DevOps Assistant": "Provides CI/CD, Docker, Kubernetes, and infrastructure guidance with configuration examples.",
    "Database Manager": "Guides schema design, query optimization, and migration planning. No direct database access.",
    "Test Generator": "Generates unit, integration, and edge case tests for your code. Test execution requires approval.",
    "Documentation Generator": "Creates API docs, READMEs, and user guides from your descriptions.",
    "Script Writer": "Generates automation scripts in Python, shell, and PowerShell. Execution requires approval.",
    "Copy Editor": "Edits text for grammar, style, clarity, and tone while preserving the author's voice.",
    "Podcast Planner": "Plans podcast episodes, seasons, show notes, and segment structures.",
    "Brand Strategist": "Develops brand identity, positioning, values, and personality for businesses and products.",
    "Presentation Coach": "Coaches presentation structure, talking points, and delivery for meetings and talks.",
    "PR Assistant": "Drafts press releases, media pitches, and crisis response statements. Distribution requires approval.",
    "Internal Comms Writer": "Writes company announcements, team updates, and change management messages.",
    "Academic Citation Manager": "Formats citations in APA, MLA, Chicago, and other academic styles.",
    "Patent Researcher": "Guides prior art search, patent claim analysis, and research strategies. Informational only.",
    "Market Analyst": "Analyzes market size, trends, competitors, and opportunities for business planning.",
    "Recipe Planner": "Suggests meals, recipes, and weekly meal prep plans based on your preferences.",
    "Travel Planner": "Creates travel itineraries with activities, logistics, and budget tips.",
    "Event Planner": "Plans events with timelines, logistics, checklists, and contingency strategies.",
    "Personal Finance Manager": "Provides budgeting, debt repayment, and savings guidance. General advice only — not financial advice.",
    "Privacy Compliance Checker": "Checks GDPR, CCPA, and privacy regulation compliance. Informational only — consult a legal professional.",
    "Data Governance Advisor": "Advises on data governance frameworks, quality metrics, and stewardship roles.",
    "Curriculum Designer": "Designs courses and curricula with learning objectives, modules, and assessments.",
    "Exam Prep Coach": "Creates study plans, practice strategies, and exam preparation coaching.",
}
...
class CapabilitySelectionDialog(QDialog):
    ...
    def __init__(self, use_case: UseCaseClass, parent=None, current_selections: list = None):
        ...
        try:
            ...
            self._membership_tier = MembershipTier.TRIAL
    ...
    def _get_compatible_caps(self, capability: str) -> list:
        ...
        # Check registry directly first, then fall back to canonical lookup
        cap_data = CAPABILITY_REGISTRY.get(capability)
        if not cap_data:
...
class CharacterSheetWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._caps_placeholder = QLabel("Capabilities for the chosen Use-Case Class appear here — tick what this AI should have. 'Suggest Set' auto-ticks the recommended ones.")
    ...
    def _enforce_capability_limit(self, state):
        ...
        try:
            from ...core.membership_tiers import (
                get_capability_limit, get_effective_tier,
                get_upgrade_prompt_for_limit, MembershipTier as _MT,
            )
            tier = get_effective_tier()
            try:
                from ...core.license_manager import get_license_manager
                if get_license_manager().is_founder_mode:
                    tier = _MT.ALL_ROUNDER
            except Exception:
                pass
            limit = get_capability_limit(tier)
            if limit >= 0:
                checked = [chk for chk in self._cap_checks if chk.isChecked()]
                if len(checked) > limit:
                    ...
                    QMessageBox.warning(
                        self, "Capability Limit Reached",
                        get_upgrade_prompt_for_limit(tier),
                    )
        finally:
            self._enforcing_limit = False
    ...
    def _save_ai(self):
        try:
            ...
            # ── Resource gate check: verify system can handle this AI's capabilities ──
            est_ram = 256 + len(capabilities) * 128
            est_cpu = 0.5 + len(capabilities) * 0.2
            est_load = min(0.9, 0.1 + len(capabilities) * 0.08)
            gate_result = self._resource_gate.check_can_activate(
                ram_mb=est_ram, cpu_cores=est_cpu, load_score=est_load,
                name=f"AI: {name} ({len(capabilities)} capabilities)",
            )
            if gate_result.decision == GateDecision.DENY:
                QMessageBox.critical(
                    self, "BLOCKED — Resource Limit Exceeded",
                    f"Cannot create AI '{name}' with {len(capabilities)} capabilities.\n\n"
                    f"{gate_result.message}\n\n"
                    f"Reduce the number of capabilities or deactivate other AIs/modules first."
                )
                return
            if gate_result.decision == GateDecision.WARN:
                reply = QMessageBox.warning(
                    self, "Warning — High Resource Load",
                    f"{gate_result.message}\n\nCreate this AI anyway?",
            ...
            QMessageBox.critical(
                self, "Save Failed",
                f"Could not save AI '{name}' to the Forge.\n\nError: {e}"
            )
...
class AIForgeWindow(QMainWindow):
    ...
    voice_command_forwarded = Signal(str, str)  # ai_name, text — emitted when voice command needs forwarding
    ...
    def _ensure_starter_ai(self):
        ...
        # Locked starter list — 2 capabilities each for all non-FREE-trial tiers
        starters = [
            {
                "name": "Lily",
                "use_case": UseCaseClass.INDIVIDUAL,
                "capabilities": ["Chat Companion", "Creative Writer"],
                "guardrails": [
                    "Ask before editing files",
                    "Cite sources when researching",
                    "Keep responses beginner-friendly",
                ...
                "notes": "Friendly chat companion and creative writer — your starting point for conversation, drafting, and basic assistance.",
            },
            {
                "name": "Daedalus",
                ...
                "libraries": ["Code Safety Library", "Research Discipline Library"],
                "personality": {"creativity": 45, "formality": 55, "caution": 75},
                "notes": "Coding helper and research assistant — asks before file edits, explains reasoning, looks things up.",
            },
            {
                "name": "Hephaestus",
                "use_case": UseCaseClass.TASK_READY,
                ...
                "notes": "Design-brief assistant — structures purpose, constraints, unknowns, scale, and materials into handoff-ready briefs.",
            },
        ]

        # Full starter list — original capabilities, only used during active FREE trial
        starters_full = [
            {
                "name": "Lily",
                "use_case": UseCaseClass.INDIVIDUAL,
        ...
        # Select which starter list to use based on membership tier and trial status
        from ...core.membership_tiers import is_trial_active
        use_full = False
        try:
            from ...core.settings_manager import SettingsManager as _SM
            _tier_val = _SM().get().membership_tier
            from ...core.membership_tiers import MembershipTier as _MT
            _tier = _MT(_tier_val)
            if _tier == MembershipTier.FREE and is_trial_active():
                use_full = True
        except Exception:
            use_full = False
        ...
        try:
            _cap_limit = get_capability_limit(get_effective_tier())
        except Exception:
            _cap_limit = 3
        if _cap_limit >= 0:
            for tpl in active_starters:
                if len(tpl["capabilities"]) > _cap_limit:
                    tpl["capabilities"] = tpl["capabilities"][:_cap_limit]
        ...
        for tpl in active_starters:
            ...
            if existing is not None:
                ...
                # Force repair for capability changes — starter capabilities are always locked
                # regardless of book_defaults_edited, since the tier structure requires it
                cap_changed = existing.capabilities != tpl["capabilities"]
                if needs_repair and (cap_changed or not getattr(existing, "book_defaults_edited", False)):
    ...
    def _setup_ui(self):
        ...
        btn_create = QPushButton("Create New AI from Scratch")
        btn_create.setObjectName("forge_create_button")
        btn_create.setStyleSheet("background-color: #2e7d32; color: white; font-weight: bold;")
        btn_create.clicked.connect(self._create_new_ai)
        ...
        btn_export_req = QPushButton("Submit Export Request to Support")
        btn_export_req.setStyleSheet("background-color: #455a64; color: white;")
        btn_export_req.clicked.connect(self._submit_export_request)
    ...
    def _open_capability_dialog(self, unit: AIUnit, dlg_name: str):
        ...
        # T2: Wire voice command signal from capability dialogs
        if hasattr(dlg, 'voice_command_ready'):
            dlg.voice_command_ready.connect(lambda text: self._on_voice_command(unit, text))
        dlg.exec()

    def _on_voice_command(self, unit: AIUnit, text: str):
        """Handle voice command from a capability dialog — forward via signal."""
        self.voice_command_forwarded.emit(unit.name, text)
        self._audit_event("voice_command", msg=f"unit={unit.name}, text={text[:80]}")

    def _create_new_ai(self):
        """Reset the character sheet form for creating a new AI from scratch."""
        # License enforcement
        allowed, msg = self._check_can_create_ai()
        if not allowed:
            QMessageBox.warning(self, "License Limit", msg)
            self._audit_event("create_denied", msg=f"tier={self._license.get_tier_label()}, reason=limit")
            return
        ...
        # Check if AI Factory is locked by Approved Use Locks
        allowed, lock_msg = check_use_lock(UseLockArea.AI_FACTORY)
        if not allowed:
            QMessageBox.warning(self, "Approved Use Locks", lock_msg)
            return

        # Reset the character sheet form
        self._sheet._reset_form()
        # Clear any selection in the list so the user knows they're starting fresh
        self._list.clearSelection()
        self._selected_ai = None

        # Update the detail panel
        self._detail.setPlainText(
            "=== Create New AI from Scratch ===\n\n"
            "Fill out the character sheet on the left:\n"
            "  1. Enter an AI name\n"
            "  2. Select a use-case class\n"
            "  3. Choose capabilities\n"
            "  4. Adjust personality traits\n"
            "  5. Select guardrails and libraries\n"
            "  6. Add any context notes\n\n"
            "Then click 'Save AI to Forge' at the bottom of the form.\n\n"
            "The AI will be checked against your system's available resources\n"
            "before it is created."
        )

        self._audit_event("create_new_ai_started", msg="User started create-from-scratch flow")
    ...
    def _submit_export_request(self):
        """Submit an export request to support for review.

        This does NOT process the export immediately. It creates a review
        request file that goes under review by support staff. The user
        is informed that their request is pending and will be reviewed.
        """
        item = self._list.currentItem()
        if not item:
            QMessageBox.warning(self, "No Selection", "Select an AI to submit an export request.")
            return
        uid = item.data(Qt.ItemDataRole.UserRole)
        ...
        if ai_source != "DROPPED_IN":
            ...
            self._audit_event("export_request_denied", msg=f"{target.name}: not a dropped-in AI (source={ai_source})")
            return

        # Load import record if exists
        records_dir = self._store_dir / "import_records"
        records_dir.mkdir(parents=True, exist_ok=True)
        record_path = None
        record_data = None
        ...
        # Load book content if available (for the review package)
        book_text = ""
        ...
        # ── Create export review request ──
        # This is a REQUEST, not an immediate export. It goes to support for review.
        export_requests_dir = self._store_dir / "export_requests"
        export_requests_dir.mkdir(parents=True, exist_ok=True)

        request_id = f"EXP-{uuid.uuid4().hex[:8].upper()}"
        request_data = {
        ...
        request_file = export_requests_dir / f"{request_id}.json"
        try:
            request_file.write_text(json.dumps(request_data, indent=2, default=str), encoding="utf-8")
        except Exception as e:
            QMessageBox.critical(self, "Request Failed", f"Could not create export request:\n{e}")
            return

        # Update import record status to UNDER_REVIEW
        if record_path and record_data:
            try:
                record_data["status"] = ImportStatus.UNDER_REVIEW.value
                record_data["export_request_id"] = request_id
                record_path.write_text(json.dumps(record_data, indent=2, default=str), encoding="utf-8")
            except Exception:
                pass

        # Inform the user
        QMessageBox.information(
            self, "Export Request Submitted",
            f"Export request for '{target.name}' has been submitted to support for review.\n\n"
            f"Request ID: {request_id}\n"
            f"Status: PENDING REVIEW\n\n"
            f"This request will be reviewed by support staff. The AI will be scanned for "
            f"malicious content, harmful instructions, and policy violations before any "
            f"export is approved.\n\n"
            f"You will be notified when the review is complete. Do not delete this AI "
            f"while the request is pending."
        )
        self._audit_event("export_request_submitted", msg=f"{target.name}: request_id={request_id}")
