...
try:
    ...
    HAS_TTS = False
from PySide6.QtGui import QImage, QPixmap, QKeyEvent, QTextCursor, QFont
...
# ---------------------------------------------------------------------------
# Book Encryption helpers — shared implementation in src/core/book_crypto.py
# ---------------------------------------------------------------------------
...
class AuditSimulator(QObject):
    ...
    thought_updated = Signal(str)
    action_updated = Signal(str)
    trajectory_updated = Signal(str)
...
class ControlBar(QWidget):
    ...
    redirect_clicked = Signal()
    demonstrate_clicked = Signal()
...
class VoiceController(QObject):
    """
    OS-native text-to-speech using TTSEngine (Windows SAPI, macOS say, Linux espeak).
    No GPL dependencies. Runs speech on a background thread so the UI stays responsive.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self._enabled = False
...
class SpeechRecognizer(QObject):
    """
    Speech-to-text using local faster-whisper (MIT license, no cloud API).
    ...
    def _init_whisper(self):
        """Initialize local Whisper model (faster-whisper preferred, openai-whisper fallback)."""
        try:
            from faster_whisper import WhisperModel
            for p in self.WHISPER_MODEL_PATHS:
                if p.exists():
                    self._whisper_model = WhisperModel(str(p), device="cpu", compute_type="int8")
    ...
    def start_continuous(self):
        """T4: Start continuous listening mode."""
        if not self._has_sr or (self._thread and self._thread.is_alive()):
            return
        self._stop_event.clear()
        self._thread = threading.Thread(target=self._continuous_worker, daemon=True)
        self._thread.start()
    ...
    def _listen_worker(self):
        ...
        try:
            ...
            try:
                ...
                self.error_occurred.emit("sounddevice not available for microphone input")
                return

            fs = 16000
            # T7: VAD-based recording — record in chunks, stop on silence
    ...
    def _continuous_worker(self):
        """T4: Continuous listening — transcribe in 2-second chunks."""
        self.listening_changed.emit(True)
        try:
            import numpy as np
            import sounddevice as sd
            ...
            self.error_occurred.emit(f"Continuous STT error: {e}")
        finally:
            self.listening_changed.emit(False)
...
class NavigationBar(QWidget):
    ...
    open_forge = Signal()
    open_book = Signal()
    open_constraints = Signal()
    open_governance = Signal()
    open_customer_ai = Signal()
    open_upgrades = Signal()
    open_license = Signal()
    open_themes = Signal()
    open_models = Signal()
    open_knowledge = Signal()
    open_voice = Signal()
    open_scheduler = Signal()
    ...
    def __init__(self, parent=None):
        ...
        nav_btn_style = "font-weight: bold; padding: 4px 8px;"
        btn_forge.setStyleSheet(f"background-color: #5e35b1; color: white; {nav_btn_style}")
        btn_book.setStyleSheet(f"background-color: #00897b; color: white; {nav_btn_style}")
        btn_constraints.setStyleSheet(f"background-color: #f57c00; color: white; {nav_btn_style}")
        btn_governance.setStyleSheet(f"background-color: #455a64; color: white; {nav_btn_style}")
        btn_customer_ai.setStyleSheet(f"background-color: #2e7d32; color: white; {nav_btn_style}")
        btn_tour.setStyleSheet(f"background-color: #1f6feb; color: white; {nav_btn_style}")
        btn_license.setStyleSheet(f"background-color: #6a4c93; color: white; {nav_btn_style}")
        btn_themes.setStyleSheet(f"background-color: #e91e63; color: white; {nav_btn_style}")
        btn_models.setStyleSheet(f"background-color: #0064a8; color: white; {nav_btn_style}")
        btn_knowledge.setStyleSheet(f"background-color: #8b5cf6; color: white; {nav_btn_style}")
        btn_voice_panel.setStyleSheet(f"background-color: #f0883e; color: white; {nav_btn_style}")
        btn_scheduler.setStyleSheet(f"background-color: #6a4c93; color: white; {nav_btn_style}")
        ...
        self._btn_voice.setStyleSheet(f"background-color: #30363d; color: #8b949e; {nav_btn_style}")
        self._btn_voice.clicked.connect(self._on_voice_toggle)

        self._btn_mic = QPushButton("Mic")
        self._btn_mic.setStyleSheet(f"background-color: #1f6feb; color: white; {nav_btn_style}")
        self._btn_mic.clicked.connect(self.mic_clicked.emit)

        # G10: Voice activity LED indicator
        self._voice_led = QLabel("●")
        self._voice_led.setStyleSheet("color: #484f58; font-size: 16px; min-width: 20px;")
        self._voice_led.setToolTip("Voice inactive")
        ...
        layout.addWidget(self._voice_led)
        layout.addStretch()
        ...
        # ── Resource Gate status indicator ──
        try:
            from ...core.resource_gate import get_resource_gate
            self._resource_gate = get_resource_gate()
            self._btn_resource = QPushButton("Load: 0%")
            self._btn_resource.setStyleSheet(
                "background-color: #4caf50; color: white; font-weight: bold; "
                "padding: 4px 8px; border-radius: 4px;"
            )
            self._btn_resource.setToolTip("System resource load — click to refresh")
            self._btn_resource.clicked.connect(self._refresh_resource_status)
            layout.addWidget(self._btn_resource)
            ...
            self._btn_resource = None

    def _refresh_resource_status(self):
        """Update the resource gate status button."""
        if not hasattr(self, '_btn_resource') or self._btn_resource is None:
            return
        try:
            status = self._resource_gate.get_detailed_status()
            load = status['cumulative_load']
            grade = status['grade']
            active = status['active_count']
            gpu_mode = status.get('gpu_mode', 'unknown')
            self._btn_resource.setText(f"Load: {load:.0%} | {active} active")
            colors = {
                'green': '#4caf50',
                'green_yellow': '#cddc39',
                'yellow': '#ffeb3b',
            ...
            color = colors.get(grade, '#4caf50')
            text_color = 'white' if grade in ('green', 'yellow_red', 'red', 'crimson_red') else '#0d1117'
            self._btn_resource.setStyleSheet(
                f"background-color: {color}; color: {text_color}; font-weight: bold; "
                f"min-width: 100px; border-radius: 4px;"
            )
            snap = status['snapshot']
            if status.get('has_dedicated_gpu'):
                vram_line = f"VRAM: {snap.available_vram_mb} MB free / {snap.total_vram_mb} MB (dedicated GPU)"
            else:
                vram_line = f"VRAM: shared with system RAM (integrated graphics — GPU workloads use CPU+RAM)"
            self._btn_resource.setToolTip(
                f"System Resource Load\n"
                f"GPU: {gpu_mode}\n"
                f"Load: {load:.0%} ({grade})\n"
                f"Active capabilities: {active}\n"
                f"RAM: {snap.available_ram_mb} MB free / {snap.total_ram_mb} MB\n"
    ...
    def set_voice_active(self, active: bool):
        """G10: Toggle LED indicator color based on listening state."""
        if active:
            self._voice_led.setStyleSheet("color: #238636; font-size: 16px; min-width: 20px;")
            self._voice_led.setToolTip("Listening...")
        else:
            self._voice_led.setStyleSheet("color: #484f58; font-size: 16px; min-width: 20px;")
            self._voice_led.setToolTip("Voice inactive")
...
class QuickActionsGrid(QWidget):
    """Easy Mode dropdown of one-click capability actions.

    Shows a dropdown (QComboBox) with one entry per capability the selected AI has.
    Selecting an entry opens SimpleCapabilityLauncher — a child-friendly
    ...
    def __init__(self, parent=None):
        ...
        header.setStyleSheet("color: #58a6ff; font-size: 13px; font-weight: bold; padding: 4px 0;")
        ...
        self._combo.setStyleSheet(
            "QComboBox { background-color: #0f172a; color: #e2e8f0; "
            "border: 1px solid #334155; border-radius: 6px; padding: 6px 12px; "
            "font-size: 13px; font-weight: bold; }"
            "QComboBox::drop-down { border: none; }"
            "QComboBox QAbstractItemView { background-color: #0f172a; color: #e2e8f0; "
            "selection-background-color: #1a73e8; selection-color: white; }"
        )
        self._combo.addItem("Select an AI above to see quick actions...")
        self._combo.currentIndexChanged.connect(self._on_combo_changed)
        row.addWidget(self._combo, stretch=1)
        self._go_btn = QPushButton("Go")
        self._go_btn.setMinimumHeight(32)
        self._go_btn.setStyleSheet(
            "QPushButton { background-color: #1a73e8; color: white; font-weight: bold; "
    ...
    def set_capabilities(self, capabilities: list[str], parent_window, ai_uuid: str, ai_name: str,
        ...
        if not capabilities:
            ...
            self._combo.addItem(f"\u26a1 Choose a quick action ({len(capabilities)} available)...")
            for cap in capabilities:
                qs = get_quick_start(cap)
                self._combo.addItem(f"{qs['emoji']}  {qs['title']}")
            self._go_btn.setEnabled(True)
        self._combo.blockSignals(False)

    def _on_combo_changed(self, index: int):
        if index <= 0 or not self._capabilities:
            self._go_btn.setEnabled(False)
        else:
            self._go_btn.setEnabled(True)
    ...
    def _on_go_clicked(self):
        ...
        if idx <= 0 or idx > len(self._capabilities):
            return
        self._on_button_click(self._capabilities[idx - 1])
...
class VisibilityWindow(QMainWindow):
    ...
    def __init__(self, router=None, registry=None, audit=None, approval=None, watcher=None):
        ...
        self._init_responsive_geometry()
    ...
    def _init_responsive_geometry(self):
        """Size window to 95% of monitor usable work area, centered."""
        try:
    ...
    def resizeEvent(self, event):
        """Collapse right panel at narrow widths; restore at wider widths."""
        ...
        if w < 1050:
            if self._splitter.sizes()[1] > 0:
                self._splitter.setSizes([w - 10, 0])
        else:
            if self._splitter.sizes()[1] < 200:
                self._splitter.setSizes([max(w - 270, 600), 250])
    ...
    def _setup_ui(self):
        ...
        # Splitter — main workspace expands, right column stays ~250px
...
"""
    ...
    def _on_quick_chat(self):
        ...
        # M6: one Workspace Chat per deployed AI — focus/raise the existing window
    ...
    def connect_watcher(self, watcher):
        ...
        # Poll scan count every 2 seconds — skip in DEV mode (nothing to poll)
        self._watcher_poll = QTimer(self)
        self._watcher_poll.timeout.connect(lambda: self._update_watcher_detail(watcher))
        watcher_mode_val = watcher.get_mode() if hasattr(watcher, "get_mode") else ""
        if watcher_mode_val != "dev":
            self._watcher_poll.start(2000)
        # Set passive label if in passive or dev mode
        if watcher_mode_val in {"STABILIZATION", "REPAIR", "CREATION", "DEMO"}:
        ...
        elif watcher_mode_val == "dev":
            self._watcher_trust_label.setText("Protection: DEV (log-only)")
            self._watcher_trust_label.setStyleSheet(
                "color: #58a6ff; font-weight: bold; font-size: 13px; "
                "padding: 2px 10px; background-color: #0d1117; border-radius: 4px;"
            )
    ...
    def _on_mic_clicked(self):
        if not self._mic.available:
            QMessageBox.information(
                ...
                "Install it with:  pip install faster-whisper sounddevice numpy\n"
                "Ensure model files exist at b:/local_models/faster-whisper-small.en/"
            )
            return
    ...
    def _on_mic_text(self, text: str):
        # T19: Voice command shortcuts
        lower = text.strip().lower()
        if lower == "stop":
            self._mic.stop_listening()
            self._thought_pane.append("[VOICE] Stop command received.")
            self._nav._btn_mic.setEnabled(True)
            self._nav._btn_mic.setText("Mic")
            return
        if lower == "clear":
            self._task_input.clear()
            self._thought_pane.append("[VOICE] Clear command received.")
            self._nav._btn_mic.setEnabled(True)
            self._nav._btn_mic.setText("Mic")
            return
        if lower == "start mission":
            self._task_input.setText(text)
            self._thought_pane.append(f"[USER] {text}")
            self._speak("Starting mission.")
    ...
    def _on_mic_listening(self, listening: bool):
        if not listening:
            self._nav._btn_mic.setEnabled(True)
            self._nav._btn_mic.setText("Mic")
        self._nav.set_voice_active(listening)
    ...
    def keyPressEvent(self, event: QKeyEvent):
        """Hidden owner console trigger: Ctrl+Shift+O. Push-to-talk: Ctrl+Shift+M."""
        if (
            ...
            event.accept()
            return
        # G4: Ctrl+Shift+M = push-to-talk
        if (
            event.key() == Qt.Key.Key_M
            and event.modifiers() == (Qt.KeyboardModifier.ControlModifier | Qt.KeyboardModifier.ShiftModifier)
        ):
            if hasattr(self, '_mic') and self._mic and self._mic.available:
    ...
    def closeEvent(self, event):
        ...
        try:
            if hasattr(self, '_mic') and self._mic:
                self._mic.stop_listening()
        except Exception:
            pass
