...
class UpgradesDialog(QDialog):
    ...
    def _contact_sales(self, upgrade_id: str):
        """Show an inquiry form that submits to Supabase and notifies the owner."""
        import os as _os
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle(f"Contact Sales — {name}")
        dlg.setFixedWidth(480)
        dlg_layout = QVBoxLayout(dlg)

        dlg_layout.addWidget(QLabel(f"<h3>Enterprise Inquiry: {name}</h3>"))
        dlg_layout.addWidget(QLabel("Fill out the form below and we'll get back to you within 24 hours."))

        dlg_layout.addWidget(QLabel("Your Name:"))
        name_input = QLineEdit()
        name_input.setPlaceholderText("John Smith")
        name_input.setStyleSheet("color: #c9d1d9; border: 1px solid #30363d; padding: 6px;")
        dlg_layout.addWidget(name_input)

        dlg_layout.addWidget(QLabel("Your Email:"))
        email_input = QLineEdit()
        email_input.setPlaceholderText("john@company.com")
        email_input.setStyleSheet("color: #c9d1d9; border: 1px solid #30363d; padding: 6px;")
        dlg_layout.addWidget(email_input)

        dlg_layout.addWidget(QLabel("Organization:"))
        org_input = QLineEdit()
        org_input.setPlaceholderText("Company name")
        org_input.setStyleSheet("color: #c9d1d9; border: 1px solid #30363d; padding: 6px;")
        dlg_layout.addWidget(org_input)

        dlg_layout.addWidget(QLabel("Number of Seats Needed:"))
        seats_input = QLineEdit()
        seats_input.setPlaceholderText("e.g. 10")
        seats_input.setStyleSheet("color: #c9d1d9; border: 1px solid #30363d; padding: 6px;")
        dlg_layout.addWidget(seats_input)

        dlg_layout.addWidget(QLabel("Use Case / Requirements:"))
        use_case_input = QTextEdit()
        use_case_input.setPlaceholderText("Tell us about your needs...")
        use_case_input.setStyleSheet("color: #c9d1d9; border: 1px solid #30363d; padding: 6px;")
        use_case_input.setMaximumHeight(100)
        dlg_layout.addWidget(use_case_input)

        def _submit_inquiry():
            user_name = name_input.text().strip()
            user_email = email_input.text().strip()
            org = org_input.text().strip()
            seats = seats_input.text().strip()
            use_case = use_case_input.toPlainText().strip()

            if not user_name or not user_email or not use_case:
                QMessageBox.warning(dlg, "Missing Information", "Please fill in your name, email, and use case.")
                return
            ...
            def _send():
                ...
                error_msg = ""
                try:
                    supabase_url = _os.environ.get("CN_SUPABASE_URL", "")
                    supabase_key = _os.environ.get("CN_SUPABASE_API_KEY", "")
                    payload = {
                        "name": user_name,
                        "email": user_email,
                        "company": org,
                        "team_size": seats,
                        "use_case": f"[Command Nexus Desktop — {name}]\n{use_case}",
                    }
                    if supabase_url and supabase_key:
                        req = _ureq.Request(
                            f"{supabase_url}/functions/v1/sales-inquiry-notify",
                    ...
                    if _os.name == "nt":
                        _sp.Popen(
                            ["powershell", "-NoProfile", "-Command",
                             ...
                             "$n.Visible = $true; "
                             f"$n.ShowBalloonTip(5000, 'New Sales Inquiry', 'From: {user_name} ({user_email}) — {name}', [System.Windows.Forms.ToolTipIcon]::Info); "
                ...
                if success:
                    ...
                    QMessageBox.information(
                        self,
                        "Inquiry Submitted",
                        f"Thank you, {user_name}! Your inquiry has been submitted.\n"
                        f"We'll contact you at {user_email} within 24 hours.",
                    )
                else:
                    dlg.accept()
                    QMessageBox.information(
                        self,
                        "Inquiry Saved",
                        f"Thank you, {user_name}! We couldn't reach our server right now, "
                        f"but your inquiry has been noted.\n\n"
                        f"Please also email us directly at AveryLogicWorks@gmail.com "
                        f"to ensure we receive it.",
                    )

            _th.Thread(target=_send, daemon=True).start()

        btn_row = QHBoxLayout()
        btn_submit = QPushButton("Submit Inquiry")
        btn_submit.setStyleSheet(
            "background-color: #238636; color: white; font-weight: bold; "
            "padding: 8px 20px; border-radius: 4px;"
        )
        btn_submit.clicked.connect(_submit_inquiry)
        btn_row.addWidget(btn_submit)

        btn_email = QPushButton("Email Directly")
        btn_email.setStyleSheet(
            "background-color: #1f6feb; color: white; font-weight: bold; "
            "padding: 8px 20px; border-radius: 4px;"
        )
        btn_email.clicked.connect(lambda: (
            dlg.accept(),
            webbrowser.open(
                f"mailto:AveryLogicWorks@gmail.com?subject={urllib.parse.quote(f'Enterprise Inquiry: {name}')}"
                f"&body={urllib.parse.quote(f'I am interested in {name} for my organization.\n\nName: {name_input.text()}\nEmail: {email_input.text()}\nOrg: {org_input.text()}\nSeats: {seats_input.text()}\n\nUse case:\n{use_case_input.toPlainText()}')}"
            ),
        ))
