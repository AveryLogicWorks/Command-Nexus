...
from .constraints_models import (
    CapabilityModule, UpgradeTier, ResourceGrade, SystemSnapshot
)
...
class ResourceGradeBar(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._grade = ResourceGrade.GREEN
        self._pct = 0.0
        self.setFrameShape(QFrame.Shape.StyledPanel)
        self.setStyleSheet("background-color: #4caf50; border-radius: 4px;")

    def set_grade(self, grade: ResourceGrade, pct: float):
        self._grade = grade
        self._pct = pct
        colors = {
            ResourceGrade.GREEN: "#4caf50",
            ResourceGrade.GREEN_YELLOW: "#cddc39",
            ResourceGrade.YELLOW: "#ffeb3b",
            ResourceGrade.YELLOW_RED: "#ff9800",
            ResourceGrade.RED: "#f44336",
            ResourceGrade.CRIMSON_RED: "#b71c1c",
        }
        self.setStyleSheet(
...
class ModuleCard(QFrame):
    ...
    activated = pyqtSignal(object)  # CapabilityModule
...
class ConstraintsWindow(QMainWindow):
    ...
    def _on_module_activated(self, module: CapabilityModule):
        ...
        snapshot = self._sys_monitor.get_snapshot()

        # Cumulative check
        total_score = sum(m.get_selected_tier().load_score for m in self._modules if m.active)

        if tier.load_score >= 0.80:  # Crimson Red
            # Auto-deactivate and block
            for card in self._cards:
                if card.get_module().id == module.id:
                    card.force_deactivate()
            QMessageBox.critical(
                self, "BLOCKED — Danger Zone",
                f"'{module.name}' at tier '{tier.name}' is in the CRIMSON RED zone.\n\n"
                f"This activation is blocked to prevent harm to your system or data.\n"
                f"System would be at {total_score:.0%} total load.\n\n"
                f"Select a lower tier or free up resources."
            )
            return

        if tier.load_score >= 0.60:  # Red
            reply = QMessageBox.warning(
                self, "Warning — Red Zone",
                f"Are you sure you want to do this?\n\n"
                f"'{module.name}' at tier '{tier.name}' is in the RED zone.\n"
                f"This could cause errors or crash your computer.\n"
                f"If it doesn't crash, it will be extremely slow.\n\n"
                f"System would be at {total_score:.0%} total load.\n\n"
                f"Proceed anyway?",
    ...
    def _recalculate(self):
        ...
        if not active:
            ...
            self._cum_bar.set_grade(ResourceGrade.GREEN, 0.0)
            return

        total_score = sum(m.get_selected_tier().load_score for m in active)
        total_ram = sum(m.get_selected_tier().ram_mb for m in active)
        snap = self._sys_monitor.get_snapshot()

        if total_score <= 0.15:
            grade = ResourceGrade.GREEN
        elif total_score <= 0.30:
            grade = ResourceGrade.GREEN_YELLOW
        elif total_score <= 0.45:
            grade = ResourceGrade.YELLOW
        elif total_score <= 0.60:
            grade = ResourceGrade.YELLOW_RED
        elif total_score <= 0.80:
            grade = ResourceGrade.RED
        else:
            grade = ResourceGrade.CRIMSON_RED
        ...
        self._cum_bar.set_grade(grade, total_score)

        # If crimson red is reached, warn and suggest deactivation
        if grade == ResourceGrade.CRIMSON_RED:
            QMessageBox.critical(
                self, "SYSTEM OVERLOAD",
                f"CRIMSON RED reached: {total_score:.0%} total load.\n\n"
                f"Some modules have been force-deactivated to protect your system."
            )
            # Deactivate highest-load modules until under crimson
            active_sorted = sorted(active, key=lambda m: m.get_selected_tier().load_score, reverse=True)
            for m in active_sorted:
                ...
                for card in self._cards:
                    if card.get_module().id == m.id:
                        card.force_deactivate()
                        total_score -= m.get_selected_tier().load_score
                        break
            self._recalculate()
