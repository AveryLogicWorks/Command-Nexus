...
BASELINE_GUARDRAILS = [
    ...
    GuardrailRule(
        ...
        keywords=[
            ...
            "zero day", "remote access trojan", "stealer", "wiper"
        ],
        phrases=[
            "how to hack",
            "create malware",
    ...
    # =========================================================================
    # COMPUTER HARM - Damaging computers, destroying data, system sabotage
    # =========================================================================

    GuardrailRule(
        id="computer_harm",
        name="Computer & Data Harm",
        description="Instructions for damaging computers, destroying data, or sabotaging systems",
        category=GuardrailCategory.SECURITY,
        severity=SeverityLevel.HIGH,
        keywords=[
            "destroy computer", "damage computer", "brick computer", "fry hard drive",
            "corrupt hard drive", "wipe all data", "destroy data", "delete everything",
            "overwrite mbr", "destroy motherboard", "short circuit computer",
            "usb killer", "destructive payload", "data destruction tool",
        ],
        phrases=[
            "how to destroy a computer",
            "how to brick a computer",
            "how to wipe all data",
            "how to corrupt a hard drive",
            "how to damage a computer",
            "destroy all data on",
            "permanently damage computer",
            "make a usb killer",
            "destroy motherboard",
        ],
        block_message="I cannot provide instructions for damaging computers, destroying data, or sabotaging systems.",
        alert_admin=True
    ),

    # =========================================================================
    # HARASSMENT & STALKING - Targeted harassment, doxxing, stalking
    # =========================================================================

    GuardrailRule(
        id="harassment_stalking",
        name="Harassment & Stalking",
        description="Instructions for targeted harassment, stalking, or doxxing individuals",
        category=GuardrailCategory.HARMFUL,
        severity=SeverityLevel.HIGH,
        keywords=[
            "doxxing", "dox someone", "stalk someone", "harass someone",
            "cyberbully", "cyber bullying", "targeted harassment",
            "reveal address", "reveal phone number", "expose personal info",
            "swatting", "harassment campaign", "online stalking",
        ],
        phrases=[
            "how to dox someone",
            ...
            "expose someone's personal information",
            "harassment campaign against",
            "how to swat someone",
        ],
        regex_patterns=[
            r"\b(?:dox|doxx)\s+(?:someone|a\s+person|people)\b",
            r"\b(?:stalk|harass|bully)\s+(?:someone|a\s+person|people)\s+online\b",
            r"\b(?:find|reveal|expose|get)\s+(?:someone'?s|a\s+person'?s)\s+(?:home\s+)?(?:address|phone\s+number|real\s+name|location)\b",
        ],
        block_message="I cannot assist with harassment, stalking, doxxing, or exposing someone's personal information. These actions are illegal and harmful.",
        alert_admin=True,
        flag_for_review=True
    ),

    # =========================================================================
    # ANIMAL CRUELTY - Abuse, torture, or harm to animals
    # =========================================================================

    GuardrailRule(
        id="animal_cruelty",
        name="Animal Cruelty",
        description="Instructions for abusing, torturing, or harming animals",
        category=GuardrailCategory.HARMFUL,
        severity=SeverityLevel.HIGH,
        keywords=[
            "animal abuse", "animal cruelty", "torture animal", "harm animal",
            "kill animal", "abuse pet", "abuse dog", "abuse cat",
            "animal fighting", "dog fighting", "cockfighting",
            "poison animal", "drown animal", "starve animal",
        ],
        phrases=[
            "how to abuse an animal",
            "how to torture an animal",
            "how to harm an animal",
            "animal fighting setup",
            "how to poison an animal",
            "animal cruelty instructions",
            "hurt a dog",
            "hurt a cat",
            "animal fighting ring",
        ],
        block_message="I cannot provide instructions for harming, abusing, or torturing animals. Animal cruelty is illegal and morally reprehensible.",
        alert_admin=True,
        flag_for_review=True
    ),

    # =========================================================================
    # EXTORTION & BLACKMAIL - Threats to reveal information unless demands met
    # =========================================================================

    GuardrailRule(
        id="extortion_blackmail",
        name="Extortion & Blackmail",
        description="Instructions for extorting, blackmailing, or coercing people through threats",
        category=GuardrailCategory.ILLEGAL,
        severity=SeverityLevel.CRITICAL,
        keywords=[
            "blackmail", "extort", "extortion", "ransom demand",
            "threaten to expose", "threaten to reveal", "pay or else",
            "coerce payment", "leverage threat", "demand money or",
        ],
        phrases=[
            "how to blackmail someone",
            "how to extort someone",
            "threaten to expose unless",
            "pay me or i'll reveal",
            "demand money or else",
            "how to coerce someone",
            "leverage information for money",
            "threaten to release information",
        ],
        regex_patterns=[
            r"\b(?:blackmail|extort)\s+(?:someone|a\s+person|people)\b",
            r"\b(?:pay|money|ransom)\s+.{0,10}(?:or\s+(?:else|i'?ll|we'?ll)\s+(?:expose|reveal|release|leak|publish))\b",
            r"\b(?:threaten\s+to\s+(?:expose|reveal|release|leak))\s+.{0,15}(?:unless|if\s+not|or)\b",
        ],
        block_message="I cannot assist with extortion, blackmail, or coercing people through threats. These are serious criminal offenses.",
        alert_admin=True,
        flag_for_review=True
    ),

    # =========================================================================
    # UNETHICAL ACTIVITY - General catch-all for unethical requests
    # =========================================================================

    GuardrailRule(
        id="unethical_activity",
        name="Unethical Activity",
        description="Requests for unethical, exploitative, or morally reprehensible activities not covered by other rules",
        category=GuardrailCategory.HARMFUL,
        severity=SeverityLevel.MEDIUM,
        keywords=[
            "unethical", "exploit people", "exploit vulnerability",
            "manipulate people", "deceive people for profit",
            "scam people", "defraud people", "take advantage of",
            "predatory", "exploitative scheme", "con people",
        ],
        phrases=[
            "how to exploit people",
            "how to scam people",
            "how to defraud people",
            "how to manipulate people for profit",
        ...
        regex_patterns=[
            r"\b(?:exploit|scam|defraud|con)\s+(?:people|person|individuals|customers|users|vulnerable)\b",
            r"\b(?:predatory|exploitative)\s+(?:scheme|practice|business|model|tactic)\b",
            r"\b(?:take\s+advantage\s+of|manipulate\s+for\s+profit)\s+(?:people|person|individuals|customers|users|vulnerable|elderly|children)\b",
        ],
        block_message="I cannot assist with unethical, exploitative, or predatory activities designed to harm or deceive people.",
        alert_admin=True
    ),
]
...
class BaselineGuardrailEngine:
    ...
    def check_content(self, text: str, context: str = "general") -> Tuple[bool, Optional[GuardrailRule], str]:
        ...
        for rule in self.rules:
            ...
            # Check keywords (word-boundary match: prevents short keywords like
            # "RAT" from matching inside "generate", "operate", "collaborate")
            if any(
