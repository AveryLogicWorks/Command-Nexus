...
class CommandNexusApp:
    def __init__(self):
        ...
        # ── Resource Gate ────────────────────────────────────────────────
        # Central resource monitor that gates capability activation across
        # ALL windows. Prevents system overload by tracking CPU, RAM, VRAM,
        # and disk usage. Every window checks this before activating capabilities.
        try:
            self._resource_gate = get_resource_gate()
            self._audit.log(
                tool="CommandNexusApp",
                action="RESOURCE_GATE_INIT",
                target="Central resource gate initialized",
                approved=True,
                status="info",
            )
        except Exception as e:
            try:
                self._audit.log(tool="CommandNexusApp", action="RESOURCE_GATE_ERROR", target=str(e), approved=False, status="error")
            except Exception:
                pass
        ...
        # Secret key check — without CN_SECRET_KEY the license system cannot
        ...
        try:
            ...
            # Port already bound (or unusable) — most commonly a second instance
            QMessageBox.warning(
                None,
                "Already Running",
        ...
        # Mandatory security update check — blocks launch if version is below minimum
        try:
            from src.core.update_checker import check_mandatory_update
            if not check_mandatory_update(parent=self._visibility):
                self._cleanup()
                sys.exit(0)
        except Exception:
            pass

        # Beta disclaimer — shown once on first run of new version
        try:
            s = self._settings.get()
            if not s.beta_disclaimer_shown:
                from PySide6.QtWidgets import QMessageBox as _QMB
                _QMB.information(
                    self._visibility,
                    "Beta Update Notice",
                    "<h2>During beta, some forced updates may be necessary.</h2>"
                    "<p>We apologize for the inconvenience. Command Nexus is still under active development, "
                    "and this update fixes critical entitlement and licensing issues to protect your data and ensure fair access.</p>"
                    "<p><b>What changed:</b></p>"
                    "<ul>"
                    "<li>Fixed a critical issue where Starter AI capabilities were not properly restricted by tier</li>"
                    "<li>3-day free trial now correctly enforces Pro-level access during trial, and locks down after expiry</li>"
                    "<li>Backend entitlement enforcement added — capabilities are now checked at execution, not just UI</li>"
                    "<li>Enterprise Contact Sales button now works properly</li>"
                    "</ul>"
                    "<p>Thank you for your patience during the beta period.</p>",
                )
                self._settings.update(beta_disclaimer_shown=True)
        except Exception:
            pass

        # Trial expiry enforcement — check if FREE 3-day trial has expired
        try:
            self._license.enforce_trial_expiry()
        except Exception as e:
            # Never fail silently — log the error for diagnosis
        ...
        try:
            s = self._settings.get()
            if s.membership_tier == 0 and not s.trial_start_date:
                from datetime import datetime as _dt
                self._settings.update(trial_start_date=_dt.now().isoformat())
        except Exception:
            pass
    ...
    def _open_forge(self):
        if self._forge is None:
            self._forge = AIForgeWindow(self._registry, self._audit, resource_gate=self._resource_gate)
            self._forge.ai_activated.connect(self._on_ai_activated)
            self._forge.book_requested.connect(self._on_book_requested)
            self._forge.voice_command_forwarded.connect(self._on_voice_command_from_forge)
        self._forge.show()
        self._forge.raise_()

    def _on_voice_command_from_forge(self, ai_name: str, text: str):
        """Forward a voice command from the Forge to Mission Control."""
        if hasattr(self, '_visibility') and self._visibility and hasattr(self._visibility, '_task_input'):
            self._visibility._task_input.setText(text)
            if hasattr(self._visibility, '_thought_pane'):
                self._visibility._thought_pane.append(f"[VOICE] {ai_name}: {text}")
            if hasattr(self._visibility, '_on_start_mission'):
                self._visibility._on_start_mission()
    ...
    def _open_book(self):
        if self._book is None:
            self._book = BookWindow(self._registry, self._audit, resource_gate=self._resource_gate)
            self._book.defaults_edited.connect(self._on_book_defaults_edited)
            self._book.command_to_ai.connect(self._route_book_command)
        self._book.show()
        self._book.raise_()
    ...
    def _open_constraints(self):
        if self._constraints is None:
            self._constraints = ConstraintsWindow(resource_gate=self._resource_gate)
    ...
    def _open_voice(self):
        ...
        # Forward voice transcription to the mission task input and auto-submit
        if hasattr(self._visibility, '_task_input'):
            def _on_voice_sent(text):
                self._visibility._task_input.setText(text)
                if hasattr(self._visibility, '_on_start_mission'):
                    self._visibility._on_start_mission()
            dlg._panel.transcription_sent.connect(_on_voice_sent)
        dlg.exec()
    ...
    def _open_customer_ai(self):
        ...
        try:
            if self._customer_ai is None:
                self._customer_ai = CustomerAIWindow(self._audit, self._visibility, resource_gate=self._resource_gate)
                self._customer_ai.escalation_needed.connect(self._on_customer_escalation)
            self._customer_ai.show()
            self._customer_ai.raise_()
        except Exception as e:
            QMessageBox.critical(self._visibility, "Support Error", f"Failed to open Customer AI support window:\n\n{e}")
            self._customer_ai = None
