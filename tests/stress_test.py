...
def test_runtime_intents_and_learning():
    ...
    try:
        ...
        # Simulate a backend that is offline by mocking call_model to return an error.
        from src.core.backend_manager import BackendResponse
        def offline_call_model(prompt, model=None, temperature=None):
            return BackendResponse(error="Backend offline (mocked for test)", provider_id="mock", display_name="Mock Offline")
        runtime._backend.call_model = offline_call_model

        # Chat should use local intelligence fallback when backend is offline.
        ...
        # Local intelligence fallback should be COMPLETED, clearly labeled as local
        ...
        def fake_call_model(prompt, model=None, temperature=None):
