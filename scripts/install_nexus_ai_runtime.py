...
RUNTIME = ROOT / "src" / "core" / "nexus_ai_runtime.py"
SMOKE = ROOT / "tools" / "smoke_test_nexus_ai_runtime.py"
FORGE = ROOT / "src" / "parts" / "forge" / "forge_window.py"
...
SMOKE.write_text(smoke_code, encoding="utf-8")
print("WROTE:", SMOKE)

# Patch Visibility Window to use NexusAIRuntime
text = VIS.read_text(encoding="utf-8")
backup = VIS.with_suffix(".py.nexus_runtime_backup")
if not backup.exists():
    shutil.copy2(VIS, backup)

if "from ...core.nexus_ai_runtime import NexusAIRuntime, RuntimeStatus as NexusRuntimeStatus" not in text:
    anchor = "from ...core.nexus_moirai import check_action_allowed, MoiraiHealthReport\n"
    text = text.replace(anchor, anchor + "from ...core.nexus_ai_runtime import NexusAIRuntime, RuntimeStatus as NexusRuntimeStatus\n", 1)

if "self._nexus_ai_runtime = NexusAIRuntime()" not in text:
    anchor = "        self._settings.initialize()\n"
    text = text.replace(anchor, anchor + "        self._nexus_ai_runtime = NexusAIRuntime()\n", 1)
...
# Make mission start stop simulator and run fast to runtime.
text = text.replace(
...
if count != 1:
    print("ERROR: Could not replace _on_mission_tick in visibility_window.py")
    sys.exit(2)
...
# Patch Forge wording so users see Knowledge/Intelligence instead of mysterious Book language.
if FORGE.exists():
    forge_backup = FORGE.with_suffix(".py.knowledge_wording_backup")
    if not forge_backup.exists():
        shutil.copy2(FORGE, forge_backup)

    ftext = FORGE.read_text(encoding="utf-8")
    ftext = ftext.replace("Ability Book", "Knowledge / Intelligence Profile")
    ftext = ftext.replace("Ability book", "Knowledge / Intelligence profile")
    ftext = ftext.replace("ability book", "Knowledge / Intelligence profile")
    ftext = ftext.replace("Book context", "Knowledge / Intelligence context")
    ftext = ftext.replace("AI Book", "AI Knowledge / Intelligence Profile")
    ftext = ftext.replace("book rendering", "Knowledge / Intelligence rendering")
    ftext = ftext.replace("book_defaults_edited", "book_defaults_edited")
    FORGE.write_text(ftext, encoding="utf-8")
    print("PATCHED WORDING:", FORGE)
    print("BACKUP:", forge_backup)
...
if FORGE.exists():
    py_compile.compile(str(FORGE), doraise=True)
print("Syntax OK.")

print()
print("Nexus AI Runtime installed.")
print("Run next:")
print(r"py .\tools\smoke_test_nexus_ai_runtime.py")
print()
print("Then relaunch:")
print(r".\launch.bat")
