...
"""Multi-Finder Registry with Reciprocal Rank Fusion.

Finders:
  1. BM25 Lexical Finder
  2. Concept Finder (abstract concept clustering)
  3. Containment Finder (TOC + path-based search)
  4. Keyword Finder (token matching with tag boost)

Each finder returns ranked results. The registry fuses them via RRF.
"""
...
class BM25Finder:
    ...
    def search(self, query: str, top_k: int = 10) -> list[FinderResult]:
        ...
        return [FinderResult(doc_id=d, score=s, finder_name="bm25") for d, s in scores[:top_k]]


class ConceptFinder:
    def __init__(self):
        self._concepts: dict[str, set[str]] = {}
        self._doc_concepts: dict[str, list[str]] = {}
    ...
    def search(self, query: str, top_k: int = 10) -> list[FinderResult]:
        ...
        for qc in q_concepts:
            for doc_id in self._concepts.get(qc, set()):
                scores[doc_id] = scores.get(doc_id, 0) + 1.0
        ranked = sorted(scores.items(), key=lambda x: -x[1])
        return [FinderResult(doc_id=d, score=s, finder_name="concept") for d, s in ranked[:top_k]]
...
class KeywordFinder:
    ...
    def search(self, query: str, top_k: int = 10) -> list[FinderResult]:
        ...
        for doc_id, content in self._docs.items():
            ct = set(_clean_tokens(content))
            overlap = len(qt & ct)
            tag_overlap = len(qt & set(t.lower() for t in self._tags.get(doc_id, [])))
            score = overlap + tag_overlap * 2.0
            if score > 0:
                scores[doc_id] = score
        ranked = sorted(scores.items(), key=lambda x: -x[1])
        return [FinderResult(doc_id=d, score=s, finder_name="keyword") for d, s in ranked[:top_k]]


class ContainmentFinder:
    def __init__(self):
        self._paths: dict[str, str] = {}
        self._toc_entries: list[tuple[str, str]] = []

    def index_page(self, page_id: str, path_string: str, toc_topic: str = "") -> None:
        self._paths[page_id] = path_string.lower()
        if toc_topic:
            self._toc_entries.append((toc_topic.lower(), page_id))

    def search(self, query: str, top_k: int = 10) -> list[FinderResult]:
        qt = set(_clean_tokens(query))
        scores: dict[str, float] = {}
        for topic, page_id in self._toc_entries:
        ...
        ranked = sorted(scores.items(), key=lambda x: -x[1])
        return [FinderResult(doc_id=d, score=s, finder_name="containment") for d, s in ranked[:top_k]]
...
class FinderRegistry:
    ...
    def add_document(self, doc_id: str, content: str,
        ...
        self.keyword.add_document(doc_id, content, tags)
    ...
    def search(self, query: str, top_k: int = 10) -> list[FusedResult]:
        ...
        all_results: list[list[FinderResult]] = [
            self.bm25.search(enriched, top_k=top_k * 2),
            self.concept.search(enriched, top_k=top_k * 2),
            self.keyword.search(query, top_k=top_k * 2),
            self.containment.search(query, top_k=top_k * 2),
        ]
    ...
    def _make_snippet(self, doc_id: str, query: str, max_len: int = 200) -> str:
        ...
        for sent in sentences:
            ...
            if overlap > best_overlap:
                ...
                best_sent = sent
        return (best_sent or content)[:max_len]
