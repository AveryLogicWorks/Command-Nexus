...
from .ai_avatar_widget import AIAvatarWidget
from ...core.avatar_config import get_avatar_assets
...
class ChatCapabilityDialog(BaseCapabilityDialog):
    def __init__(self, ai_name, ai_uuid, abilities, book_path=None, guardrails=None, libraries=None, use_case="", parent=None):
        ...
        # Left side: avatar + chat stacked vertically
        left_container = QWidget()
        left_layout = QVBoxLayout(left_container)
        left_layout.setContentsMargins(0, 0, 0, 0)
        left_layout.setSpacing(4)

        # AI Avatar widget — load video assets from config if available
        avatar_assets = get_avatar_assets()
        self._avatar = AIAvatarWidget(
            idle_image=avatar_assets.get("idle_image"),
            idle_video=avatar_assets.get("idle_video"),
            talking_video=avatar_assets.get("talking_video"),
            fallback_text=ai_name[:2].upper(),
            size=QSize(200, 280),
        )
        left_layout.addWidget(self._avatar, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        left_layout.addWidget(chat_panel, stretch=1)
    ...
    def _on_send(self):
        ...
        # Avatar state sequence: listening → thinking → talking → idle
        self._avatar.set_state("listening")
        from PySide6.QtCore import QTimer
        QTimer.singleShot(400, lambda: self._avatar.set_state("thinking"))
        ...
        # AI is now "talking" while displaying the response
        self._avatar.set_state("talking")
        self._append_ai(" ".join(reply_parts))
        self._input.clear()

        # Return to idle after response is shown
        from PySide6.QtCore import QTimer
        QTimer.singleShot(1200, lambda: self._avatar.set_state("idle"))
