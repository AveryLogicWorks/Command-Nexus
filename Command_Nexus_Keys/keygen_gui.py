...
=============================================
Single-window PySide6 GUI. Pick a key type, set params, generate.
...
def _ensure_pyside6() -> bool:
    """Auto-install PySide6 if missing. Returns True if available."""
    try:
    ...
    import subprocess
    import sys
    print("PySide6 not found. Auto-installing...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "PySide6"])
        print("PySide6 installed successfully.")
        return True
    except Exception as exc:
        print(f"Failed to auto-install PySide6: {exc}")
        return False


HAS_PYQT = _ensure_pyside6()
...
def main():
    if not HAS_PYQT:
        ...
        print("Install:  py -3.12 -m pip install PySide6")
        sys.exit(1)
