...
The `.bat` files **auto-check and auto-install** PySide6 if it's missing. No manual setup needed — just double-click and go.
...
├── keygen_gui.py                # Unified PySide6 GUI (all generators + validator)
...
- **PySide6** — auto-installed by the `.bat` files if missing. No action needed.
